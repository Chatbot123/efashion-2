...
if($method == 'POST')
{
	...
	if ($com == 'amountsold' or $com == 'margin' or $com == 'qtysold' or $com=='shoplist' or $com=='listfamily' or $com=='listcategory' or $com=='listarticle') 
	{
		...
		$json_url = "http://74.201.240.43:8000/ChatBot/Sample_chatbot/EFASHION_TEST2.xsjs?command=$com&STATE=$STATE&CITY=$CITY&SHOPNAME=$SHOPNAME&YR=$YR&QTR=$QTR&MTH=$MTH&FAMILY=$FAMILY&CATEGORY=$CATEGORY&ARTICLE=$ARTICLE";		
		//echo $json_url;
		$username    = "SANYAM_K";
		...
		if($com == 'amountsold' or $com == 'margin' or $com == 'qtysold')
		{
			...
			{
				$speech .= $distext. $value["AMOUNT"].$disshop.$value["SHOP_NAME"].$discity.$value["CITY"]." in ".$value["STATE"].$disyear.$value["YR"].$disfamily.$value["FAMILY"].$discategory.$value["CATEGORY"].$disarticle.$value["ARTICLE"];
